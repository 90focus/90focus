...
{ h: '4. Preise und Zahlung', p: 'Alle Preise sind in Euro (EUR) angegeben. Der Kaufpreis ist sofort nach Bestellung fällig und wird über die hinterlegte Zahlungsmethode abgebucht.' },
...
{ h: '4. Prices and Payment', p: 'All prices are listed in Euro (EUR). The purchase price is due immediately upon ordering and will be charged to the payment method on file.' },
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#070b0f', color: '#e8eef4', fontFamily: 'sans-serif' }}>
      <div style={{ maxWidth: 700, margin: '60px auto 0', padding: '60px 24px' }}>
        ...
        <h1 style={{ fontSize: 32, fontWeight: 900, textTransform: 'uppercase', marginBottom: 40 }}>{content.title}</h1>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
          {content.sections.map((s, i) => (
            <div key={i} style={{ background: '#0d1219', border: '1px solid #1c2a38', borderRadius: 8, padding: '24px' }}>
              <div style={{ color: '#e8ff00', fontSize: 11, fontWeight: 800, letterSpacing: 3, textTransform: 'uppercase', marginBottom: 16 }}>{s.h}</div>
              <p style={{ color: '#667788', lineHeight: 1.8, fontSize: 14 }}>{s.p}</p>
            </div>
          ))}
